...
<p>When processing markup for an object that will be constructed by a builder, the <span class="code">Builder</span> instances are treated like value objects - if a <span class="code">Builder</span> implements the <span class="code">Map</span> interface, the <span class="code">put()</span> method is used to set the builder's attribute values. Otherwise, the builder is wrapped in a <span class="code">BeanAdapter</span> and its properties are assumed to be exposed via standard Bean setters.</p>
    <div class="info-alert"><strong>HTML Analogy:</strong> In HTML, typing <code>&lt;button&gt;</code> creates an instance of <code>HTMLButtonElement</code>. In FXML, typing <code>&lt;Button&gt;</code> creates an instance of the <code>javafx.scene.control.Button</code> class. Both rely on a parser to convert markup tags into in-memory object instances.</div>
<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;!-- HTML Instance Declaration --&gt;
&lt;img src="/logo.png" alt="Logo" /&gt;
&lt;!-- This creates an HTMLImageElement in the DOM --&gt;</code></pre></div>
    </section>
    <div class="pagination">
      <router-link to="/fxml/class-instance-elements" class="btn btn-prev">❮ Class Instance Elements</router-link>
      <router-link to="/fxml/fx-include" class="btn btn-next">&lt;fx:include&gt; ❯</router-link>
    </div>
  </article>
</template>
