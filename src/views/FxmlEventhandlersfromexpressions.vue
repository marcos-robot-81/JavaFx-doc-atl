...
<div class="info-alert"><strong>HTML Analogy:</strong> Binding an event handler using an expression is equivalent to passing a function reference to an event listener. In modern web frameworks like React or Vue, you frequently pass references to functions (or expressions that evaluate to a function) directly into the component's event attribute.</div>
<div class="code-block"><div class="code-header">Web Equivalent (React)</div><pre><code>&lt;!-- React JSX --&gt;
&lt;button onClick={this.onActionHandler}&gt;Click Me!&lt;/button&gt;</code></pre></div>
    </section>
    <div class="pagination">
      <router-link to="/fxml/controller-method-event-handlers" class="btn btn-prev">❮ Controller Method Event Handlers</router-link>
      <router-link to="/fxml/special-handlers-for-collections-and-properties" class="btn btn-next">Special handlers for collections and properties ❯</router-link>
    </div>
  </article>
</template>
