...
<p>Like property elements, attributes can also be used to configure the 
properties of a class instance. For example, the following markup 
creates a <span class="code">Button</span> whose text reads "Click Me!":</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;?import javafx.scene.control.*?&gt;
&lt;Button text="Click Me!"/&gt;
</code></pre></div>

<p>As with property elements, property attributes support type coercion.
...
be converted to a <span class="code">Color</span>:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;Rectangle fx:id="rectangle" x="10" y="10" width="320" height="240"
    fill="#ff0000"/&gt;
</code></pre></div>

<p>Unlike property elements, which are applied as they are processed, 
...
processed (for example, the selected index of a <span class="code">TabPane</span> control, which can't be set until all of the tabs have been added).</p>

<p>Another key difference between property attributes and property 
elements in FXML is that attributes support a number of "resolution 
operators" that extend their functionality. The following operators are 
supported and are discussed in more detail below:</p>
...
<p>As strings, XML attributes cannot natively represent typed location 
...
<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;ImageView image="@my_image.png"/&gt;
</code></pre></div>
...
<p>Note that whitespace values in the URL must be encoded; for example, 
to refer to a file named "My Image.png", the FXML document should 
contain the following:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;Image url="@My%20Image.png"/&gt;
</code></pre></div>

<p>rather than:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;Image url="@My Image.png"/&gt;
</code></pre></div>

<h4><a id="resource_resolution">Resource Resolution</a></h4>

<p>In FXML, resource substitution can be performed at load time for localization purposes. When provided with an instance of <span class="code">java.util.ResourceBundle</span>,
 the FXML loader will replace instances of resource names with their 
locale-specific values. Resource names are identified by a "%" prefix, 
as shown below:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;Label text="%myText"/&gt;
</code></pre></div>

<p>If the loader is given a resource bundle defined as follows:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>myText = This is the text!
</code></pre></div>
...
<p>An FXML document defines a variable namespace in which named elements
 and script variables may be uniquely identified. The variable 
resolution operator allows a caller to replace an attribute value with 
an instance of a named object before the corresponding setter method is 
invoked. Variable references are identified by a "$" prefix, as shown 
below:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;fx:define&gt;
...
<p>Assigning an <span class="code">fx:id</span> value to an element 
creates a variable in the document's namespace that can later be 
referred to by variable dereference attributes, such as the 
"toggleGroup" attribute shown above, or in script code, discussed in a 
later section. Additionally, if the object's type defines an "id" 
property, this value will also be passed to the objects <span class="code">setId()</span> method.</p>

<h4><a id="escape_sequences">Escape Sequences</a></h4>

<p>If the value of an attribute begins with one of the resource 
resolution prefixes, the character can be escaped by prepending it with a
 leading backslash ("\") character. For example, the following markup 
creates a <span class="code">Label</span> instance whose text reads "$10.00":</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;Label text="\$10.00"/&gt;
</code></pre></div>

<h4><a id="expression_binding">Expression Binding</a></h4>
<p>Attribute variables as shown above are resolved once at load time. 
Later updates to the variables value are not automatically reflected in 
any properties to which the value was assigned. In many cases, this is 
...
<p>An expression binding also begins with the variable resolution 
operator, but is followed by a set of curly braces which wrap the 
expression value. For example, the following markup binds the value of a
 text input's "text" property to the "text" property of a <span class="code">Label</span> instance:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;TextField fx:id="textField"/&gt;
...
<table>
 ...
 <tbody><tr><th scope="col">Constant / Operator</th><th scope="col">Description</th></tr>
 ...
 </tr><tr><th scope="row">! <br>(unary operator)</th><td>Unary negation of a boolean</td></tr>
 <tr><th scope="row">+ - <br>
                        * /
...
<p>Attributes representing static properties are handled similarly to 
static property elements and use a similar syntax. For example, the 
earlier <span class="code">GridPane</span> markup shown earlier to demonstrate static property elements could be rewritten as follows:</p>
...
<p>In addition to being more concise, static property attributes, like 
...
to create an expression binding to a static property.</p>
...
<p>FXML supports three types of event handler attributes: script event 
...
the user presses the button:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;?language javascript?&gt;
...
...
<p>Note the use of the language processing instruction at the beginning 
of the code snippet. This PI tells the FXML loader which scripting 
language should be used to execute the event handler. A page language 
must be specified whenever inline script is used in an FXML document, 
and can only be specified once per document. However, this does not 
apply to external scripts, which may be implemented using any number of 
supported scripting languages. Scripting is discussed in more detail in 
the next section.</p>
...
<p><strong>Note:</strong> The JavaScript script engine is disabled by 
default. If the JDK has a JavaScript script engine, it can be enabled 
using a system property <span class="code">javafx.allowjs=true.</span></p>

<h4><a id="controller_method_event_handlers">Controller Method Event Handlers</a></h4>
<p>A controller method event handler is a method defined by a document's
...
elements) defined by the document.</p>
...
<p>In general, a handler method should conform to the signature of a 
...
argument often carries important and useful information about the nature
 of the event; however, it is optional and may be omitted if desired.
...
<p>Note that collections and properties do not currently support scripting handlers.</p>
      <div class="info-alert"><strong>HTML Analogy:</strong> In HTML, elements use attributes for configuring layout, linking resources, assigning IDs/classes, and hooking up event handlers (e.g., <code>onclick="handleClick()"</code>).</div>
      <div class="code-block"><div class="code-header">Example</div><pre><code class="language-html">&lt;!-- HTML Analogy for Attributes in General --&gt;
...
&lt;/button&gt;
    ...
    <div class="pagination">
      <router-link to="/fxml/define-blocks" class="btn btn-prev">❮ Define Blocks</router-link>
      <router-link to="/fxml/instance-properties" class="btn btn-next">Instance Properties ❯</router-link>
    </div>
  </article>
</template>
