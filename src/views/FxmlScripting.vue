...
<p>Script code may also be defined in external files. The previous 
example could be split into an FXML file and a JavaScript source file 
with no difference in functionality:</p>
...
<div class="code-block"><div class="code-header">Example</div><pre><code>
function handleButtonAction(event) {
   java.lang.System.out.println('You clicked me!');
}
</code></pre></div>

<p>It is often preferable to separate code from markup in this way, 
since many text editors support syntax highlighting for the various 
scripting languages supported by the JVM. It can also help improve 
readability of the source code and markup.</p>


<p>Note that script blocks are not limited to defining event handler 
...
defines a variable named "labelText". The value of this variable is used
 to populate the text property of a <span class="code">Label</span> instance:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>&lt;fx:script&gt;
...
<p><strong>Warning:</strong> As of JavaFX 8, <span class="code">importClass()</span>
 javascript function is no longer supported. You have to use fully 
qualified names as in the example above or load a nashorn compatibility 
script.</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>load("nashorn:mozilla_compat.js");
...
<p>
  In HTML/Web development, FXML's <code>&lt;fx:script&gt;</code> tag behaves just like the standard <code>&lt;script&gt;</code> tag. Inline scripts and external scripts using the <code>source</code> attribute map directly to <code>&lt;script&gt;...&lt;/script&gt;</code> and <code>&lt;script src="..."&gt;&lt;/script&gt;</code>. Binding an event handler using <code>onAction</code> works similarly to an inline <code>onclick</code> attribute.
</p>
<div class="code-block"><div class="code-header">Web Equivalent</div><pre><code>&lt;!-- Inline Script --&gt;
&lt;script&gt;
  function handleButtonAction(event) {
...
&lt;button onclick="handleButtonAction(event)"&gt;Click Me!&lt;/button&gt;
    ...
    <div class="pagination">
      <router-link to="/fxml/special-handlers-for-collections-and-properties" class="btn btn-prev">❮ Special handlers for collections and properties</router-link>
      <router-link to="/fxml/controllers" class="btn btn-next">Controllers ❯</router-link>
    </div>
  </article>
</template>
