...
<p>The <span class="code">FXMLLoader</span> class is responsible for 
...
element is assumed to be a subclass of <span class="code">javafx.scene.layout.Pane</span>, and the document is assumed to define a controller of type <span class="code">MyController</span>:</p>

<div class="code-block"><div class="code-header">Example</div><pre><code>URL location = getClass().getResource("example.fxml");
...
<p>Note that the output of an <span class="code">FXMLLoader#load()</span> operation is an instance hierarchy that reflects the actual named classes in the document, not <span class="code">org.w3c.dom</span> nodes representing those classes. Internally, <span class="code">FXMLLoader</span> uses the <span class="code">javax.xml.stream</span> API (also known as the <i>Streaming API for XML</i>, or <i>StAX</i>)
 to load an FXML document. StAX is an extremely efficient event-based 
XML parsing API that is conceptually similar to its W3C predecessor, 
SAX. It allows an FXML document to be processed in a single pass, rather
 than loaded into an intermediate DOM structure and then post-processed.</p>
...
<div class="info-alert"><strong>HTML Analogy:</strong> The FXMLLoader acts like the browser's HTML parser (or the fetch API combined with DOM parsing) that reads an HTML string or file, instantiates the corresponding DOM nodes, and binds them to your JavaScript modules or controllers.</div>
<div class="code-block"><div class="code-header">Example</div><pre><code>async function loadComponent() {
  ...
  document.body.appendChild(doc.body.firstChild);
    ...
    <div class="pagination">
      <router-link to="/fxml/nested-controllers" class="btn btn-prev">❮ Nested Controllers</router-link>
      <router-link to="/fxml/custom-components" class="btn btn-next">Custom Components ❯</router-link>
    </div>
  </article>
</template>
