...
&lt;/FXCollections&gt;
    ...
    public Builder&lt;?&gt; getBuilder(Class&lt;?&gt; type);
      ...
      <div class="info-alert">
        <strong>Required Import:</strong> To use <code>&lt;Color&gt;</code>, remember to add <code>&lt;?import javafx.scene.paint.Color?&gt;</code> at the top of your FXML file.
      </div>
      <div class="code-block">
        <div class="code-header">Example</div>
...
&lt;/Color&gt;
      ...
      <p>
        The <span class="code">&lt;fx:include&gt;</span> tag creates an object
        from FXML markup defined in another file. It is used as follows:
      </p>
      ...
      <p>
        ...
        considered relative to the path of the current document.
      </p>
...
&lt;Button text="My Button"/&gt;
      ...
      <p>
        the resulting scene graph would contain a
        <span class="code">VBox</span> as a root object with a single
        <span class="code">Button</span> as a child node.
      </p>
      ...
      <p>
        <span class="code">&lt;fx:include&gt;</span> also supports attributes
      ...
      <p>
        The <span class="code">&lt;fx:constant&gt;</span> element creates a
        reference to a class constant. For example, the following markup sets
        the value of the "minWidth" property of a<span class="code"
          >Button</span
        >
        instance to the value of the
        <span class="code">NEGATIVE_INFINITY</span> constant defined by the
        <span class="code">java.lang.Double</span> class:
      </p>

      <div class="info-alert">
        <strong>Required Import:</strong> To use <code>&lt;Button&gt;</code>, remember to add <code>&lt;?import javafx.scene.control.Button?&gt;</code> at the top of your FXML file.
      </div>
      <div class="code-block">
        <div class="code-header">Example</div>
        <pre><code>&lt;Button&gt;
...
&lt;/Button&gt;
      ...
      <p>
        The <span class="code">&lt;fx:reference&gt;</span> element creates a new
        reference to an existing element. Wherever this tag appears, it will
        effectively be replaced by the value of the named element. It is used in
        conjunction with either the <span class="code">fx:id</span> attribute or
        with a script variables, both of which are discussed in more detail in
        later sections. The "source" attribute of the
        <span class="code">&lt;fx:reference&gt;</span> element specifies the
        name of the object to which the new element will refer.
      </p>

      <p>
        For example, the following markup assigns a previously-defined
        <span class="code">Image</span> instance named "myImage" to the "image"
        property of an <span class="code">ImageView</span> control:
      </p>
...
&lt;/ImageView&gt;
      ...
      <p>
        ...
        <a href="#attributes">Attributes</a> section),
        <span class="code">fx:reference</span> is generally only used when a
        reference value must be specified as an element, such as when adding the
...
&lt;/ArrayList&gt;
      ...
      <p>
        The <span class="code">&lt;fx:copy&gt;</span> element creates a copy of
        an existing element. Like
        <span class="code">&lt;fx:reference&gt;</span>, it is used with the
        fx:id attribute or a script variable. The element's "source" attribute
        specifies the name of the object that will be copied. The source type
        must define a copy constructor that will be used to construct the copy
      ...
      <p>
        The <span class="code">&lt;fx:root&gt;</span> element creates a
        reference to a previously defined root element. It is only valid as the
        root node of an FXML document.
        <span class="code">&lt;fx:root&gt;</span> is used primarily when
        creating custom controls that are backed by FXML markup. This is
        discussed in more detail in the
        <a href="#fxmlloader">FXMLLoader</a> section.
      </p>
      ...
      <p>
        For example, the following FXML creates an instance of the
        <span class="code">Label</span> class and sets the value of the label's
        "text" property to "Hello, World!":
      </p>
...
&lt;Label text="Hello, World!"/&gt;
      ...
      <p>
        The FXML loader uses the <span class="code">coerce()</span> method of
        <span class="code">BeanAdapter</span> to perform any required type
        conversions. This method is capable of performing basic primitive type
        conversions such as <span class="code">String</span> to
        <span class="code">boolean</span> or <span class="code">int</span> to
        <span class="code">double</span>, and will also convert
        <span class="code">String</span> to <span class="code">Class</span> or
        <span class="code">String</span> to <span class="code">Enum</span>.
        Additional conversions can be implemented by defining a static
        <span class="code">valueOf()</span> method on the target type.
      </p>

      <h4>
        <a id="read_only_list_property_elements">Read-Only List Properties</a>
      </h4>
      <p>
        A read-only list property is a Bean property whose getter returns an
        instance of <span class="code">java.util.List</span> and has no
        corresponding setter method. The contents of a read-only list element
        are automatically added to the list as they are processed.
      </p>

      <p>
        For example, the "children" property of
        <span class="code">javafx.scene.Group</span> is a read-only list
        property representing the group's child nodes:
      </p>
...
&lt;/Group&gt;
      ...
      <JavaFxPreview>
        <div style="position: relative; width: 340px; height: 260px; background: #eee; border: 1px dashed #ccc;">
          <div style="position: absolute; left: 10px; top: 10px; width: 320px; height: 240px; background-color: rgb(255, 0, 0);"></div>
        </div>
      </JavaFxPreview>

      <p>
        As each sub-element of the
        <span class="code">&lt;children&gt;</span> element is read, it is added
        to the list returned by <span class="code">Group#getChildren()</span>.
      </p>
      <h4>
        <a id="read_only_map_property_elements">Read-Only Map Properties</a>
      </h4>
      <p>
        A read-only map property is a bean property whose getter returns an
        instance of <span class="code">java.util.Map</span> and has no
        corresponding setter method. The attributes of a read-only map element
        are applied to the map when the closing tag is processed.
      </p>

      <p>
        The "properties" property of
        <span class="code">javafx.scene.Node</span> is an example of a read-only
        map property. The following markup sets the "foo" and "bar" properties
        of a <span class="code">Label</span> instance to "123" and "456",
        respectively:
      </p>
...
&lt;/Button&gt;
      ...
      <JavaFxPreview>
        <button class="javafx-btn">Button (properties: foo="123", bar="456")</button>
      </JavaFxPreview>

      <p>
        Note that a read-only property whose type is neither a
        <span class="code">List</span> nor a <span class="code">Map</span> will
        be treated as if it were a read-only map. The return value of the getter
        method will be wrapped in a <span class="code">BeanAdapter</span> and
        can be used in the same way as any other read-only map.
      </p>

      <h4><a id="default_properties">Default Properties</a></h4>
      <p>
        A class may define a "default property" using the
        <span class="code">@DefaultProperty</span> annotation defined in the
        <span class="code">javafx.beans</span> package. If present, the
      ...
      <p>
        For example, since
        <span class="code">javafx.scene.layout.Pane</span> (the superclass of
        <span class="code">javafx.scene.layout.VBox</span>) defines a default
        property of "children", a
        <span class="code">&lt;children&gt;</span> element is not required; the
        loader will automatically add the sub-elements of the
        <span class="code">VBox</span> to the container's "children" collection:
      </p>
...
&lt;/VBox&gt;
      ...
      <p>
        For example, since
        <span class="code">javafx.scene.control.ScrollPane</span> defines a
        default property of "content", a scroll pane containing a
        <span class="code">TextArea</span> as its content can be specified as
        follows:
      </p>
      <div class="info-alert">
        <strong>Required Import:</strong> To use <code>&lt;ScrollPane&gt;</code> and <code>&lt;TextArea&gt;</code>, remember to add <code>&lt;?import javafx.scene.control.ScrollPane?&gt;</code> and <code>&lt;?import javafx.scene.control.TextArea?&gt;</code> at the top of your FXML file.
      </div>
      <div class="code-block">
        <div class="code-header">Example</div>
        <pre><code>&lt;ScrollPane&gt;
...
&lt;/ScrollPane&gt;
      ...
      <JavaFxPreview>
        <div style="width: 200px; height: 100px; border: 1px solid #ccc; overflow: auto; padding: 5px; background: #fff;">
          <textarea style="width: 100%; height: 100%; border: none; resize: none; outline: none; font-family: sans-serif; color: #1a1a1a;">Once upon a time...</textarea>
        </div>
      </JavaFxPreview>
      ...
      <p>
        ...
        For example, The following FXML invokes the static setter for
        <span class="code">GridPane</span>'s "rowIndex" and "columnIndex"
        properties:
      </p>

      <div class="info-alert">
        <strong>Required Import:</strong> To use <code>&lt;GridPane&gt;</code> and <code>&lt;Label&gt;</code>, remember to add <code>&lt;?import javafx.scene.layout.GridPane?&gt;</code> and <code>&lt;?import javafx.scene.control.Label?&gt;</code> at the top of your FXML file.
      </div>
      <div class="code-block">
        <div class="code-header">Example</div>
        <pre><code>&lt;GridPane&gt;
...
&lt;/GridPane&gt;
      ...
      <JavaFxPreview>
        <div style="display: grid; grid-template-columns: auto; grid-template-rows: auto; border: 1px dashed #ccc; padding: 10px; background: #fafafa;">
...
gridPane.getChildren().add(label);
      ...
      <p>
        The calls to <span class="code">GridPane#setRowIndex()</span> and
        <span class="code">GridPane#setColumnIndex()</span> "attach" the index
        data to the <span class="code">Label</span> instance.
        <span class="code">GridPane</span> then uses these during layout to
        arrange its children appropriately. Other containers, including
        <span class="code">AnchorPane</span>,
        <span class="code">BorderPane</span>, and
        <span class="code">StackPane</span>, define similar properties.
      </p>
      ...
      <p>
        The <span class="code">&lt;fx:define&gt;</span> element is used to
        create objects that exist outside of the object hierarchy but may need
        to be referred to elsewhere.
      </p>

      <p>
        For example, when working with radio buttons, it is common to define a
        <span class="code">ToggleGroup</span> that will manage the buttons'
        selection state. This group is not part of the scene graph itself, so
        should not be added to the buttons' parent. A define block can be used
        to create the button group without interferering with the overall
        structure of the document:
      </p>

      <div class="info-alert">
        <strong>Required Import:</strong> To use <code>&lt;VBox&gt;</code>, <code>&lt;ToggleGroup&gt;</code> and <code>&lt;RadioButton&gt;</code>, remember to add <code>&lt;?import javafx.scene.layout.VBox?&gt;</code>, <code>&lt;?import javafx.scene.control.ToggleGroup?&gt;</code> and <code>&lt;?import javafx.scene.control.RadioButton?&gt;</code> at the top of your FXML file.
      </div>
      <div class="code-block">
        <div class="code-header">Example</div>
        <pre><code>&lt;VBox&gt;
...
&lt;/VBox&gt;
      ...
      <JavaFxPreview>
        <div class="javafx-vbox" style="align-items: flex-start; gap: 8px;">
          <label style="display: flex; align-items: center; gap: 5px; color: #1a1a1a;"><input type="radio" name="myToggleGroup"> A</label>
          <label style="display: flex; align-items: center; gap: 5px; color: #1a1a1a;"><input type="radio" name="myToggleGroup"> B</label>
          <label style="display: flex; align-items: center; gap: 5px; color: #1a1a1a;"><input type="radio" name="myToggleGroup"> C</label>
        </div>
      </JavaFxPreview>
      ...
      <div class="info-alert">
        <strong>HTML Analogy:</strong> Like HTML tags that instantiate DOM nodes
        (e.g. <code>&lt;div&gt;</code>, <code>&lt;input&gt;</code>), FXML
        elements define class instances. FXML also allows elements to define
        properties, similar to how some HTML elements act as configuration for
        their parents (e.g. <code>&lt;source&gt;</code> inside
        <code>&lt;video&gt;</code>).
      </div>
      <div class="code-block">
        <div class="code-header">Example</div>
...
&lt;/form&gt;</code></pre>
    ...
    <div class="pagination">
      <router-link to="/fxml/overview" class="btn btn-prev"
        >❮ Overview</router-link
      >
      <router-link to="/fxml/class-instance-elements" class="btn btn-next"
